...
router.get('/feed',isLoggedIn, async function(req, res) {
  ...
  res.render("feed", {
    ...
    posts,
    stories: filtered,
    dater: utils.formatTimeDifference,
  });
...
router.get("/follow/:userid", isLoggedIn, async function (req, res) {
  ...
  if (followKarneWaala.followings.indexOf(followHoneWaala._id) !== -1) {
    let index = followKarneWaala.followings.indexOf(followHoneWaala._id);
    followKarneWaala.followings.splice(index, 1);
...
router.get('/save/:id', isLoggedIn, async function(req, res) {
  ...
  if(user.saved.indexOf(req.params.id) === -1){
    ...
    var index = user.saved.indexOf(req.params.id);
